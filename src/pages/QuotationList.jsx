...
import { Plus, FileText, Printer, CheckCircle, XCircle, Copy, FileEdit, Trash2 } from 'lucide-react'
...
export default function QuotationList() {
  ...
  async function fetchQuotations() {
    ...
    // 承認者名を別途取得（requested_approver_id / approved_by）
    const ids = [...new Set(
  ...
  async function handleDelete() {
    ...
    setDeleting(true)
    try {
      // この見積を参照している複製元リンクを解除
      await supabase
        .from('quotations')
        .update({ source_quotation_id: null })
        .eq('source_quotation_id', targetId)
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        ...
        >
          絞り込み{activeFilterCount > 0 && <span className="bg-blue-600 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center">{activeFilterCount}</span>}
        </button>
        {activeFilterCount > 0 && (
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200 py-16 text-center">
          ...
          <p className="text-gray-500 text-sm">{quotations.length === 0 ? '見積書がありません' : '条件に一致する見積書がありません'}</p>
          {quotations.length === 0 && (
            <button
              onClick={() => navigate('/quotations/new')}
                ...
                return (
                  <tr key={q.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => navigate(`/quotations/${q.id}/edit`)}>
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className={`text-xs px-2 py-0.5 rounded-full ${tl.cls}`}>
                        {tl.text}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-sm mx-4 p-6">
            ...
            <p className="text-xs text-gray-500 mb-5">
              {duplicateModal.quotation_number}「{duplicateModal.title}」
            </p>
            <div className="space-y-3">
              <button
                onClick={() => handleDuplicate('revision')}
              ...
              >
                ...
                <div className="text-xs text-blue-700">
                  ...
                  例：{duplicateModal.base_number || duplicateModal.quotation_number}-
                  {(duplicateModal.revision_number || 1) + 1}
                </div>
              </button>
              <button
                onClick={() => handleDuplicate('new')}
                disabled={duplicating}
